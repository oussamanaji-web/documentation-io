<template>
    <div v-if="userAdmin !== 'admin'">
        <h3>{{ documentation.casTitle }}</h3>
        <p>{{ documentation.casDescription }}</p>
    </div>
    <div v-else>
        <h3>Formulaire</h3>
        <form v-on:submit.prevent="submitForm" class="mt-4">
            <div class="mb-3">
                ...
                <input type="text" v-model="form.casTitle" class="form-control" id="title"
                       placeholder="Titre">
            </div>
            <div class="mb-3">
                <label for="Description" class="form-label">Description</label>
                <textarea class="form-control" v-model="form.casDescription" id="Description" placeholder="Description"
                          rows="3"></textarea>
            </div>
            <div class="mb-3">
...
export default {
    ...
    data() {
        return {
            form: {
                casTitle: this.documentation.casTitle,
                casDescription: this.documentation.casDescription,
            },
            status: false,
        }
    },
